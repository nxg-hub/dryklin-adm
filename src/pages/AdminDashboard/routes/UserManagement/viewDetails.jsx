   ...
   const [modalConfig, setModalConfig] = useState({
    ...
    const handleBack = () => {
      ...
      const handleDeactivateClick = async () => {
        ...
        try {
          const response = await fetch(`${API_BASE_URL}/api/v1/auth/${selectedUser.id}/deactivate`, {
            method: "PUT",
            headers: {
          ...
          if (response.ok) {
            ...
            setModalConfig({
              ...
              type: "error",
              title: "Action failed",
              description: typeof result === "string" ? result : result.message || "Failed to deactivate user.",
              redirectPath: null,
            });
          }
        } catch (err) {
  ...
  setModalConfig({ ...modalConfig, show: false });
    ...
    return (
      ...
      Back 
          ...
          >
          <img className="mt-8 h-25 w-25 rounded-full" src= {selectedUser?.ProfilePic || avatar}
 alt="User Image" />
          <div>
    <h2 className="mt-5 font-bold text-2xl">{selectedUser?.firstName ||selectedUser?.companyName || selectedUser?.fullName} { selectedUser?.lastName }
    </h2>
    <p className="text-sm text-gray-400 mt-1">ID NO. : {selectedUser?.id}</p>
  </div>
          </div>
          <div className="container p-6 grid grid-cols-1 md:grid-cols-3 gap-6 mt-5 text-black">
  {[
    { label: "First Name", value: selectedUser?.firstName ||selectedUser?.companyName || firstname },
    ...
    { label: "Total no. of orders", value: selectedUser?.NumberOfOrders || 'NIL' },
  ].map(({ label, value }) => (
    <div key={label} className="group w-full">
      <h1 className="text-[#E85C13] text-2xl font-bold relative">{label}</h1>
      <div className="flex items-center gap-2 mt-3">
        ...
        />
        {copied === label && <span className="text-sm text-[#E85C13]">Copied!</span>}
      </div>
    </div>
  ))}
